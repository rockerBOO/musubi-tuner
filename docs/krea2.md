...
- LoRA loading: `--lora_weight` (one or more) and `--lora_multiplier`. LoRA is merged into the base DiT weights at load time. **Not supported with `--nvfp4`** — use the original BF16 (or `--fp8_scaled`/`--convrot_int8`) weights with `--lora_weight` instead.
- **Memory-efficient inference (fits a 24GB card):**
  ...
  - `--convrot_int8` is an alternative to fp8 for the DiT base weights (cannot be combined with `--fp8_scaled`/`--nvfp4`). Quantizes per-block Linears at load time with Hadamard rotation + int8; forward runs a fused Triton int8 GEMM (requires `triton`, falls back to a slower dequantized bf16 matmul without it). `--convrot_int8_bwd` is unused at inference (training-only).
  - `--nvfp4` loads a ComfyUI pre-quantized NVFP4 (4-bit floating point, E2M1) DiT checkpoint and runs inference with true FP4x4 tensor-core forward; requires PyTorch 2.10+ and a Blackwell GPU (compute capability 10.0+). Cannot be combined with `--fp8_scaled`/`--convrot_int8`, or with `--lora_weight`. `--nvfp4_columnwise_chunk_rows` is unused at inference (training-only).
  ...
  - `--block_swap_h2d_only` streams blocks Host→Device only (keeping a CPU master, no device→host copy), which is always safe at inference since the base weights are frozen — recommended (not required) whenever using `--blocks_to_swap`, with any quantization scheme. `--block_swap_ring_size N` sets the number of GPU ring buffers (2 = transfer/compute overlap, 1 = minimal memory).
...
- LoRAの読み込み: `--lora_weight`（1つ以上）と`--lora_multiplier`。LoRAは読み込み時にベースのDiT重みにマージされます。**`--nvfp4`とは併用できません**——`--lora_weight`を使う場合は元のBF16（または`--fp8_scaled`/`--convrot_int8`）の重みを使用してください。
- **省メモリ推論（24GBに収まります）:**
  - `--fp8_scaled`で28個のメインブロックを読み込み時に動的スケールfp8に量子化します（K2はscaled fp8のみ対応。text-fusion transformerはbf16のまま）。DiTの重みメモリがおよそ半減します。
  - `--convrot_int8`はDiTベース重みに対するfp8の代替手段です（`--fp8_scaled`/`--nvfp4`とは併用できません）。読み込み時にHadamard回転+int8で各ブロックのLinearを量子化し、forwardは融合Triton int8 GEMMで実行します（`triton`が必要。ない場合はより低速な逆量子化bf16 matmulにフォールバック）。`--convrot_int8_bwd`は推論では使用されません（学習時のみ）。
  - `--nvfp4`はComfyUIで事前量子化されたNVFP4（4-bit浮動小数点、E2M1）のDiTチェックポイントを読み込み、真のFP4x4 Tensor Core forwardで推論します。PyTorch 2.10以降とBlackwell GPU（compute capability 10.0以上）が必要です。`--fp8_scaled`/`--convrot_int8`、および`--lora_weight`とは併用できません。`--nvfp4_columnwise_chunk_rows`は推論では使用されません（学習時のみ）。
  ...
  - `--block_swap_h2d_only`はブロックをHost→Deviceのみでストリーミングします（CPUマスターを保持し、device→hostコピーを行わない）。推論ではベース重みが常に凍結されているため常に安全で、量子化方式を問わず`--blocks_to_swap`使用時には推奨されます（必須ではありません）。`--block_swap_ring_size N`でGPUリングバッファ数を設定します（2で転送と計算をオーバーラップ、1で最小メモリ）。
